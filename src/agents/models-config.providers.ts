...
// Xiaomi provider constants
const XIAOMI_BASE_URL = "https://api.xiaomi.ai/v1";
export const XIAOMI_DEFAULT_MODEL_ID = "xiaomi-default";
const XIAOMI_DEFAULT_CONTEXT_WINDOW = 128000;
...
export function normalizeProviders(params: {
  ...
  for (const [key, provider] of Object.entries(providers)) {
    ...
    if (hasModels && !normalizedProvider.apiKey?.trim()) {
      ...
      const apiKey =
        fromEnv ?? fromProfiles ?? (normalizedKey === "amazon-bedrock" ? "AWS_PROFILE" : undefined);
      if (apiKey?.trim()) {
        mutated = true;
