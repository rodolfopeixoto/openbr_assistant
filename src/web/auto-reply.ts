...
type WebInboundMsg = Parameters<typeof monitorWebInbox>[0]["onMessage"] extends (
  msg: infer M,
) => unknown
  ? M
  : never;
...
function buildMentionConfig(cfg: ReturnType<typeof loadConfig>): MentionConfig {
  const gc = cfg.messages?.groupChat;
...
function isBotMentioned(msg: WebInboundMsg, mentionCfg: MentionConfig): boolean {
...
function debugMention(
  ...
  const details = {
    from: msg.from,
    body: msg.body,
    bodyClean: msg.body.replace(/[\u200b-\u200f\u202a-\u202e\u2060-\u206f]/g, "").toLowerCase(),
...
function getSessionRecipients(cfg: ReturnType<typeof loadConfig>) {
  ...
  const recipients = Object.entries(store)
    ...
    .map(([_, entry]) => ({
      to: entry?.lastChannel === "whatsapp" && entry?.lastTo ? normalizeE164(entry.lastTo) : "",
...
export function resolveHeartbeatRecipients(
  ...
  const allowFrom =
    Array.isArray(cfg.channels?.whatsapp?.allowFrom) && cfg.channels.whatsapp.allowFrom.length > 0
      ? cfg.channels.whatsapp.allowFrom.filter((v: string | number) => v !== "*").map(normalizeE164)
      : [];
...
function getSessionSnapshot(cfg: ReturnType<typeof loadConfig>, from: string, isHeartbeat = false) {
  const sessionCfg = cfg.session;
  const scope = sessionCfg?.scope ?? "per-sender";
...
export async function monitorWebProvider(
  ...
  const groupHistoryLimit = cfg.messages?.groupChat?.historyLimit ?? DEFAULT_GROUP_HISTORY_LIMIT;
  ...
  const noteGroupMember = (conversationId: string, e164?: string, name?: string) => {
    if (!e164 || !name) return;
  ...
  const resolveGroupActivationFor = (conversationId: string) => {
    const key =
      ...
      (conversationId.startsWith("group:") ? conversationId : `whatsapp:group:${conversationId}`);
  ...
  while (true) {
    ...
    const processMessage = async (msg: WebInboundMsg) => {
      ...
      if (msg.chatType === "group") {
        ...
        if (historyWithoutCurrent.length > 0) {
          const historyText = historyWithoutCurrent
            .map((m: { sender: string; body: string; timestamp?: number }) =>
              formatAgentEnvelope({
                channel: "WhatsApp",
                from: conversationId,
      ...
      for (const replyPayload of sendableReplies) {
        ...
        try {
          ...
          if (shouldLogVerbose()) {
            const preview = replyPayload.text != null ? elide(replyPayload.text, 400) : "<media>";
    ...
    enqueueSystemEvent(`WhatsApp gateway connected${selfE164 ? ` as ${selfE164}` : ""}.`, { sessionKey: "main" });
    ...
    if (keepAlive) {
      ...
      watchdogTimer = setInterval(() => {
        ...
        if (timeSinceLastActivity > MESSAGE_TIMEOUT_MS) {
          const minutesSinceLastActivity = Math.floor(timeSinceLastActivity / 60000);
    ...
    enqueueSystemEvent(`WhatsApp gateway disconnected (status ${statusCode ?? "unknown"})`, { sessionKey: "main" });

    if (loggedOut) {
      runtime.error("WhatsApp session logged out. Run `clawdis login --provider web` to relink.");
      await closeListener();
      break;
    }
