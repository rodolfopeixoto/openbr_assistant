...
// We import the llm-task tool directly (OpenClaw-native) so standalone TUI runs can
// still use the agent's configured models via the embedded runner.
// @ts-ignore - Extension file outside rootDir
...
function createFakePluginApi(cfg: any, workspaceDir: string): OpenClawPluginApi {
...
export const handlePlanCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  const isInteractiveCli =
    (params.ctx.CommandSource as string) === "cli" &&
