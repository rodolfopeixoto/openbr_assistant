/**
 * Podman Container Engine - Rootless container support for Linux
 */
...
const log = createSubsystemLogger("ralph:podman");
...
export class PodmanEngine implements ContainerEngine {
  ...
  async isAvailable(): Promise<boolean> {
    try {
      const result = execSync("podman --version", { encoding: "utf8" });
      const match = result.match(/podman version ([\d.]+)/);
      if (match) {
  ...
  async createContainer(config: ContainerConfig): Promise<Container> {
    ...
    try {
      ...
      const args = this.buildPodmanArgs(config);
      const result = execSync(`podman create ${args.join(" ")} ${config.image}`, {
        encoding: "utf8",
      });
  ...
  async stopContainer(id: string, timeout = 30): Promise<void> {
    try {
      execSync(`podman stop -t ${timeout} ${id}`);
  ...
  async removeContainer(id: string, force = false): Promise<void> {
    try {
      ...
      execSync(`podman rm ${forceFlag} ${id}`);
  ...
  async listContainers(filters?: ContainerFilters): Promise<Container[]> {
    try {
      ...
      const result = execSync(`podman ps -a --format '{{json .}}'${filterArgs}`, {
        encoding: "utf8",
      });
  ...
  async getContainer(id: string): Promise<Container | null> {
    try {
      const result = execSync(`podman inspect ${id} --format '{{json .}}'`, { encoding: "utf8" });
  ...
  async getStats(id: string): Promise<ContainerStats> {
    try {
      const result = execSync(`podman stats ${id} --no-stream --format '{{json .}}'`, {
        encoding: "utf8",
      });
  ...
  async pullImage(image: string): Promise<void> {
    try {
      ...
      execSync(`podman pull ${image}`, { stdio: "pipe" });
  ...
  private buildPodmanArgs(config: ContainerConfig): string[] {
    const args: string[] = [];
    ...
    // Podman-specific rootless options
