    ...
    return {
        ...
        async execute(_id, params) {
            ...
            const model = (typeof params.model === "string" && params.model.trim()) ||
                ...
                primaryModel ||
                undefined;
            const authProfileId = (typeof params.authProfileId === "string" &&
            ...
            if (!provider || !model || !modelKey) {
                throw new Error(`provider/model could not be resolved (provider=${String(provider ?? "")}, model=${String(model ?? "")})`);
            }
            ...
            if (allowed && allowed.length > 0 && !allowed.includes(modelKey)) {
                throw new Error(`Model not allowed by llm-task plugin config: ${modelKey}. Allowed models: ${allowed.join(", ")}`);
            }
